...
/**
 * Simple class to compare the structure of an AVL tree with the expected
 * outcome.
 *
 * Inserts 6 values into an AVL Tree, which should cause 3 trinode
 * restructurings: - 1 double rotation (right-left) at the root when 54 is
 * inserted. - 1 single rotation (right-right) when 41 is inserted. - 1 single
 * rotation (left-left) at the root when 19 is inserted.
 *
 * A Binary Search Tree is constructed that has the expected final shape for the
 * AVL tree.
 *
